  ...
  io.on("connection", (socket) => {
    ...
    socket.on("tracking:subscribe", async ({ tripId }) => {
      try {
        ...
        socket.emit("tracking:snapshot", await getTripTracking(tripId));
      } catch (error) {
    ...
    socket.on("driver:location:update", async (payload) => {
      try {
        const location = await updateTripLocation(payload);
    ...
    socket.on("driver:trip:event", async ({ tripId, type, message }) => {
      try {
        if (!tripId) {
          throw createBadRequest("tripId is required.");
        }
        ...
        await appendTripEvent(tripId, event);
